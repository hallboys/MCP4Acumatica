...
13. **Config diagnostics.** `src/lib/preflight.ts` exposes a `runPreflight()` probe that exercises every external touch-point: `ACUMATICA_URL` reachable, OIDC discovery, Connected App `client_credentials` grant (distinguishes `invalid_client` — bad creds — from `unsupported_grant_type` — creds valid, grant disabled), tenant OData path (`/t/{tenant}/...` → 401 = exists, 404 = wrong tenant), contract API endpoint version, and a **capability probe for the DAC-based OData endpoint** (`checkDacODataEndpoint`, 0.42.0 — see below). Surfaced two places: the admin console (`/docs/admin/preflight` → on-demand diagnostic table) and the `/callback` token-exchange path (known OAuth errors like `invalid_client` / `invalid_grant` are rendered as targeted pages via `interpretTokenError()` instead of a generic 502). Only the `error` field of IdentityServer error bodies is read — other fields can echo the submitted form, which includes `client_secret`.
    ...
    **VERIFIED on hallboys 25R2 (2026-07-31): available, and an ordinary user role suffices** — no elevated *OData v4 User* role needed, so that community claim does not hold here. Two facts constrain any use of it. (1) **Entity sets are addressed by bare class name**: `PX.Objects.SO.SOOrder` → 404, `SOOrder` → 200. The docs' namespace-qualified form is the OData *type*, not the set. The service document advertises up to **three aliases per DAC** — underscored namespace path (`AA_Objects_Labels_ALAutoPrint`), bare class name (`ALAutoPrint`, canonical), and a de-prefixed variant (`AutoPrint`). (2) **4766 entity sets** on this instance — the GI-flood problem an order of magnitude worse, plus three-way naming ambiguity for the same table. So a DAC-backed read tool **must** carry a curated entity-set allowlist, mirroring the GI opt-in gate; blanket exposure is not an option. Never fetch `$metadata` from a tool at that scale. The remaining blocker before any DAC read path ships is **redaction**: `redact.ts` matches on field *names*, and physical DAC field names differ from contract-entity names, so the patterns need re-validating or PII silently stops being redacted. Per the docs it honors UI-level access rights ("users have access to the same data that is visible to them in the UI"), but community reports claim it needs an elevated *OData v4 User* role, which the official docs neither confirm nor deny — disqualifying under this server's per-user-role access model if true.
