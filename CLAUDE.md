...
13. **Config diagnostics.** `src/lib/preflight.ts` exposes a `runPreflight()` probe that exercises every external touch-point: `ACUMATICA_URL` reachable, OIDC discovery, Connected App `client_credentials` grant (distinguishes `invalid_client` — bad creds — from `unsupported_grant_type` — creds valid, grant disabled), tenant OData path (`/t/{tenant}/...` → 401 = exists, 404 = wrong tenant), contract API endpoint version, and a **capability probe for the DAC-based OData endpoint** (`checkDacODataEndpoint`, 0.42.0 — see below). Surfaced two places: the admin console (`/docs/admin/preflight` → on-demand diagnostic table) and the `/callback` token-exchange path (known OAuth errors like `invalid_client` / `invalid_grant` are rendered as targeted pages via `interpretTokenError()` instead of a generic 502). Only the `error` field of IdentityServer error bodies is read — other fields can echo the submitted form, which includes `client_secret`.
    ...
    **DAC-based OData capability probe (0.42.0).** Acumatica **2025 R1+** exposes DACs directly over OData 4.0 at `/t/{tenant}/api/odata/dac` — entities named by class (`PX.Objects.SO.SOOrder`), navigation properties (`SOLineCollection`, `BAccountByCustomerID`), no Generic Inquiry required. **This server does not use it**; reads are contract REST and GI OData serves search + the login gate. The probe exists because it is the strongest candidate fix for the contract-API `$filter` failure family (see Known Issues) — those failures are artifacts of the *contract API's filter binder*, not of OData, so a real OData 4.0 surface should filter correctly including on child collections. **The unauthenticated row makes no HTTP request** — it is an honest `skip` (same shape as `checkCallbackUrl`) that defers to the authenticated probe below it. Two open questions: does the endpoint exist here, and does a normal user's role suffice?

    **Authenticated DAC probe (0.43.0).** `POST /docs/admin/preflight/dac-probe` (CSRF + admin session) answers both for real. It borrows a **user's** stored token via the same `TokenManager` DO the MCP tools use (`DOTokenProvider`), because preflight has no token of its own. `probeDacAuthenticated()` issues **two** requests — the service root, then `DAC_PROBE_ENTITY` (`PX.Objects.SO.SOOrder`, the class name Acumatica's own docs use) — because a single entity GET cannot distinguish "endpoint absent" from "entity name guessed wrong": both are 404. Verdicts come from the pure, unit-tested `interpretDacProbe(rootStatus, entityStatus)`: root 404 → absent (`skip`); root 403 → elevated *OData v4 User* role confirmed, unusable here (`warn`); root 401 → inconclusive, most likely an expired token (`warn`); root 200 + entity 200 → `pass`, the **only** combination that passes. Neither response body is read (`body.cancel()`) — the DAC service document enumerates every DAC on the instance and can be megabytes. Only status codes cross the boundary; no record data is returned or stored. Every run is logged as an `admin_action` (`logAdminAction()`) with the target username, because the call reaches Acumatica **as that user** and lands in Acumatica's own audit trail under their name.

    **VERIFIED WORKING on hallboys 25R2 (2026-07-31):** service root + a `SOOrder` read both 200. **Caveat — the test ran as an Administrator**, which may carry the **OData v4** role implicitly, so "an ordinary role suffices" is *not* established; re-run as a non-admin. Acumatica has an explicit OData v4 role, and needing to grant it is a setup prerequisite of the same kind as `MCP Access` — **not** a blocker. **Entity sets are addressed by bare class name**: `PX.Objects.SO.SOOrder` → 404, `SOOrder` → 200 (the docs' qualified form is the OData *type*, not the set). Up to **three aliases per DAC** are advertised — underscored namespace path (`AA_Objects_Labels_ALAutoPrint`), bare class name (`ALAutoPrint`, canonical), de-prefixed variant (`AutoPrint`). The instance advertises **4766 entity sets**, which is *not* an obstacle in itself: tools address entities **by name**, exactly as `GETTER_TOOLS` and `acumatica_list_entities` already do, so the catalogue never enters the model's context. (Contrast Generic Inquiries, where `list_generic_inquiries` *is* the menu — that asymmetry is why the GI gate exists and why it does not transfer here.) The count matters only in that `$metadata` must never be fetched from a tool at that scale. **What actually remains before a DAC read path ships:** (1) **redaction** — `redact.ts` matches field *names* and DAC field names are physical, so patterns need re-validating or PII silently stops being redacted (the real blocker); (2) **row-level security** — a 200 for an admin proves the request was allowed, not that rows are filtered for a restricted user; (3) **a DAC field-name source** — the offline schema index is built from the contract API's `swagger.json` and doesn't describe DAC fields, though the same `scripts/` ingestion pattern could build one from `$metadata` offline. Per the docs it honors UI-level access rights ("users have access to the same data that is visible to them in the UI"), but community reports claim it needs an elevated *OData v4 User* role, which the official docs neither confirm nor deny — disqualifying under this server's per-user-role access model if true.

    **Why it can't self-verify — and a caveat on two neighbouring checks.** Acumatica SaaS authenticates *before* routing, so an unauthenticated request returns **401 for every path**, existent or not. Verified live on 25R2: `/api/odata/dac/`, `/api/odata/gi/`, `/api/odata/nonsense/`, `/t/NotARealTenant/api/odata/gi/`, and `/entity/Default/99.999.999` **all return 401**. An anonymous 401 therefore carries no information, and preflight has no token to do better (it runs without a user, and `client_credentials` is disabled on the Connected App). **Fixed in 0.44.0.** Previously `checkTenantPath` and `checkEndpointVersion` assumed "404 = wrong tenant / wrong version" and so reported a typo'd `ACUMATICA_TENANT` or `ACUMATICA_ENDPOINT_VERSION` as `pass`. Both now return **`warn`** on a 401 (stating that Acumatica 401s every path so the value is unverifiable without a token) and reserve `pass` for a genuine 200. The verification they can no longer fake moved to `runAuthenticatedChecks()`, run from the admin console's **Authenticated checks** form with a borrowed user token — where a 404 finally means what it always claimed to: `interpretTenantAuthed()` and `interpretEndpointAuthed()` (pure, unit-tested) return `fail` on 404, `pass` only on 200, and distinguish 403 ("tenant is real, user lacks rights") from 401 ("token expired, re-run"). Licensing is a non-issue for us: the "counts as a conventional user login, not an API user" statement is scoped to **basic authentication**, and we already call `/odata/gi` with OAuth bearer tokens (see Key Design Decision #2 on the load-bearing `api` scope).
